...
function buildStackedDaily(segments, colorMap) {
    const byDate = new Map();
...
function calcStreak(stackedDaily) {
...
function buildData() {
    ...
    const customModels = vsConfig.get('aiCustomModels') ?? [];
    const now = new Date();
    ...
    for (const project of projectList) {
        if (!cfgMap.has(project) || !cfgMap.get(project).color) {
            ...
            const updated = { ...existing, project, color };
            cfgMap.set(project, updated);
            (0, storage_1.saveProjectConfig)(updated);
        }
    }
    const colorMap = new Map(Array.from(cfgMap.entries()).map(([p, cfg]) => [p, cfg.color ?? '#888888']));
    const map = new Map();
    for (const seg of segments) {
    ...
    if (aiEnabled) {
        try {
            aiSessions = (0, aiTracker_1.loadAISessions)(customModels);
    ...
    const pomodoroByProject = {};
    try {
        for (const p of (0, storage_1.readPomodoro)().filter(p => p.date === todayStr)) {
...
// ── HTML generation ────────────────────────────────────────────────────────
function projectCards(stats, totalAll_ms, pomodoroByProject, isArchived = false) {
    if (stats.length === 0) {
        return isArchived
...
function settingsTab(data) {
    ...
    const currOpts = (sel) => currencies.map(c => `<option value="${c}" ${c === sel ? 'selected' : ''}>${c}</option>`).join('');
...
<tr>
  ...
  <td>$${m.cost_usd.toFixed(2)}</td>
    ...
    const paletteSwatches = PROJECT_PALETTE.map(c => `<div class="swatch" style="background:${c}" onclick="pickColor('${c}')"></div>`).join('');
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.command) {
            ...
            case 'setProjectColor': {
                const existing = (0, storage_1.getProjectConfig)(msg.project) ?? { project: msg.project };
                (0, storage_1.saveProjectConfig)({ ...existing, project: msg.project, color: msg.color });
                break;
            }
            case 'archiveProject': {
                const existing = (0, storage_1.getProjectConfig)(msg.project) ?? { project: msg.project };
                (0, storage_1.saveProjectConfig)({ ...existing, project: msg.project, archived: msg.archived });
                refresh();
                break;
            }
